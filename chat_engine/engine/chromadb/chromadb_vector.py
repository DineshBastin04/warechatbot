    ...
    def get_similar_question_sql(
        ...
        **kwargs,
        ...
        try:
            ...
            def fetch_doc_for_table(table_name: str, expected_db: str):
                """
                Backfill documentation for a specific table name by directly
                querying the collection. Same (table, db) contract as the main
                filtering loop below: in a dual-DB workspace, a same-named table
                on the OTHER database must never be returned here just because
                its content also happens to mention table_name.
                """
                try:
                    ...
                    for d in extracted:
                        if d.get("type") == "documentation":
                            ...
                            if table_name.lower() in content and doc_db == expected_db:
                                return d
                except Exception:
                    return None
                return None
            ...
            if matched_tables:
                ...
                for t in missing_tables:
                    expected_db = matched_table_dbs_lower.get(t.lower(), primary_db_alias)
                    back_doc = fetch_doc_for_table(t, expected_db)
                    if back_doc:
                        ...
                        if "question" in parsed_data and "sql" in parsed_data:
                            ...
                            if stored_question == question:
                                ...
                                if not scope_ok:
                                    ...
                                    continue
                                # Same "frozen literal text" reasoning as the scope check
                                # above applies to OPENQUERY quote-escaping: a stored
                                # example doesn't get the fresh-generation path's
                                # validate_openquery_literals check for free, so it's
                                # re-applied here too rather than trusting it stayed valid.
                                openquery_ok, openquery_err = self.validate_openquery_literals(sql_text)
                                if not openquery_ok:
                                    logger.warning(
                                        f" Skipping exact match for '{stored_question}' — {openquery_err}. "
                                        f"Falling back to fresh generation."
                                    )
                                    continue
