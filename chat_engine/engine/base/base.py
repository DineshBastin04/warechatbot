...
class VannaBase(ABC):
    ...
    def log(self, message: str, title: str = "Info"):
        ...
        try:
            ...
            # Console codepage (e.g. Windows cp1252) can't represent every
            # character an LLM prompt/response might contain - degrade to
            # '?' substitution instead of crashing the request.
    ...
    def extract_schedule_request(
        ...
        *,
        ...
        known_email_labels: list = None,
        **kwargs,
    ) -> dict:
        """
        Single LLM call that parses a "/"-prefixed free-text scheduling request
        (e.g. "every Monday at 9am send me open orders by email") into a
        structured schedule + resolved question.

        This is a routing/parsing aid only, same trust level as classify_intent —
        NOT a security boundary. Its output only ever feeds the same
        classify_intent → generate_sql → is_sql_valid/validate_db_scope/
        validate_openquery_literals chain the interactive chat path already runs;
        callers must never skip that chain based on this function's output alone.

        Returns a dict:
        {
          "ok": bool,
          "question": str | None,        # None if the text is a follow-up that
                                          # doesn't restate a question (e.g. "send
                                          # me this weekly") — caller resolves it
                                          # from the preceding chat turn instead.
          "channel": "chat" | "email",
          "email_address": str | None,   # only set if a raw address was stated inline
          ...
          "ambiguous": [str],   # things the model had to guess at (am/pm, an
                                 ...
                                 # agent is created, never silently assumed.
          "error": str | None,
        }
        """
        reference_time = reference_time or datetime.now().isoformat()
        known_email_labels = known_email_labels or []

        email_label_instructions = ""
        if known_email_labels:
            labels_joined = ", ".join(known_email_labels)
            email_label_instructions = (
                f"\nThe user has these named email recipients registered: {labels_joined}. "
                "If the request names one or more of them (or says \"all\"), return them in "
                "\"email_labels\", normalized to the exact registered label text, or the literal "
                "\"all\" to mean every registered address. If it just says \"me\"/\"my email\" and "
                "exactly one is registered, use that one. If none are named and more than one is "
                "registered, leave \"email_labels\" empty and add a note to \"ambiguous\" asking the "
                "user to say which recipient they mean. Never invent a label that isn't registered.\n"
            )

        system_prompt = (
            "You turn a short free-text scheduling request into structured JSON. "
            f"The current date/time is {reference_time} in the timezone {default_timezone} — "
            "treat this as authoritative \"now\"; never infer or guess the current date yourself."
            f"{email_label_instructions}\n"
            "Respond with ONLY a single JSON object (no prose, no markdown fences), matching exactly:\n"
            "{\n"
            '  "ok": true,\n'
            '  "question": "<the data question being asked, in English, or null if the\n'
            '               message is a follow-up that only refers back to prior results\n'
            '               (e.g. \\"this\\", \\"this data\\", \\"it\\") without restating one>",\n'
            ...
            '  "email this to pavan"). Do NOT treat a missing timing word as "not schedulable" —\n'
            '  an unqualified send request defaults to "date" with "run_at" set to right now\n'
            '  (the current reference time given above). Only set "ok": false when the text isn\'t\n'
            '  a request to deliver/schedule anything at all (e.g. an ordinary data question with\n'
            '  no delivery intent, or gibberish).\n'
            '- "interval": fixed repeating gap. {"seconds": <int>}. Use for "every 2 days",\n'
            '  "every 6 hours", "every 90 minutes".\n'
            '- "cron": use APScheduler CronTrigger field syntax directly for everything else —\n'
            '  it already supports comma lists ("hour": "9,17"), ranges ("hour": "9-17"),\n'
            '  weekday lists ("day_of_week": "mon-fri" or "mon,wed,fri"), and special day\n'
            '  tokens ("day": "last" for end of month, "day": "last fri" for last Friday of\n'
            '  the month, "day": "1,15"). Only include the fields the request actually\n'
            '  constrains (year/month/day/week/day_of_week/hour/minute/second).\n'
            '  NEVER use the "week" field to mean "once a week" — it constrains an ISO\n'
            '  week *number* (e.g. "week": "2" only means ISO week 2 of the year), it does\n'
            '  NOT make something recur weekly, and combined with no day_of_week it fires\n'
            '  every day, not once a week. For a bare "weekly"/"once a week" request with no\n'
            '  day named, set "day_of_week" to the weekday of the current reference time\n'
            '  above (so it recurs on that same day each week) and add a note to "ambiguous"\n'
            '  naming which day you picked.\n'
        ...
        self.log(title="Schedule Extraction Prompt", message=system_prompt)
        llm_response_tuple = self.submit_prompt(messages, **kwargs)
        ...
        self.log(title="Schedule Extraction Response", message=response_text)
        ...
        # still reach the caller so it can be rejected with a clear "not registered"
        # error; dropping it here would make an unrecognized name look identical to
        # "no name was mentioned," which can silently resolve to the wrong recipient
        # (e.g. falling back to the single registered address) instead of failing loudly.
        raw_labels = parsed.get("email_labels") or []
        email_labels = [label.strip() for label in raw_labels if isinstance(label, str) and label.strip()]
    ...
    def _parse_json_response(self, response_text: str) -> dict:
        """
        Extracts a single JSON object from an LLM response, tolerating a fenced
        ```json block or ``` block around the object as well as a raw object.
