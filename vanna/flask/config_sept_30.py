...
function showPredictions(id) {
    ...
    contentArea.innerHTML = `
        ...
        <div id="predictionsContainer" style="display: none;">
            ...
            <!--<div id="suggestedPredictionsView" style="display: none;">
                ...
                <button class="test" onclick="fetchSuggestedPredictions('${id}')">Get Suggestions</button>
    ...
    // Fetch predictions and tables (for schema explorer only)
    fetchPredictionConfigs(id);
    fetchTables(id); // Populate table dropdown for schema explorer
}
                            ...
                            li.innerHTML = `
                                <div class="workspace-item">
                                    <div class="workspace-header">
                                        <span><i class="fas fa-brain"></i>${workspace.name}</span>
...
function showAnomaly(workspaceId) {
    ...
    if (isVannaConnected) {
        ...
        toggleButton.checked = true;
        anomaliesState.textContent = 'Enabled';
        anomaliesState.classList.remove('disabled');
        anomaliesState.classList.add('enabled');
...
function toggleAnomaliesState(workspaceId) {
    ...
    anomalyForm.style.display = isEnabled && document.getElementById('viewToggle').checked ? 'none' : 'none';
    addButton.disabled = !isEnabled;
}
...
function runAnomaly(anomalyType) {
    ...
    .then(data => {
        if (data.type === "error" || data.type === "sql_error") {
            ...
            displayAnomalyResults(anomalyType.replace('_', ' ') + ' Results', data.results);
            showSuccessMessage(`${anomalyType.replace('_', ' ')} anomaly detection completed Oswin!`);
        }
    })
    .catch(error => showErrorMessage(`Error running ${anomalyType}: ${error.message}`));
}

function renderAnomalyTableAndChart(anomalyType, results) {
    ...
    if (results && results.length > 0) {
                ...
                buttons: [
                    {
                        extend: 'csvHtml5',
                        text: '⬇️ Export CSV',
                        title: `Anomaly_${anomalyType}_${getWorkspaceIdFromUrl()}`
                    }
                ]
            });
        }, 0);
        ...
        if (visualization) {
            renderAnomalyVisualization(results, anomalyType);
...
// Reuse renderAnomalyVisualization from the template
function renderAnomalyVisualization(data, anomaly_type) {
    const ctx = document.getElementById('anomalyChart')?.getContext('2d');
    if (!ctx) return; // Skip if no chart canvas
    ...
    let yAxis = headers.find(h => h.includes('count') || h.includes('value') || h.includes('len') || h === 'cnt' || h.includes('missing') || h.includes('z_score') || h.includes('anomaly_score')) || headers[1];
    let isAnomaly = headers.find(h => h.includes('anomaly') || h.includes('outlier'));
